...
class DisjunctiveDiagramsBuilder:
    ...
    def ConsumeSubtree(self, diagram: DisjunctiveDiagram, root, consumed_node):
        ...
        self.nodes_with_changed_hash = OrderedSet(
            DisjunctiveDiagramsBuilder.LitLessSortNodes(diagram.order_, self.nodes_with_changed_hash))
    ...
    def DeleteLinksToNodeInTable(delnode, diagram_):
        for node in diagram_.table_.values():
            if delnode in node.low_childs:
                ...
                print('DeleteLinksToNodeInTable deleting', delnode.vertex_id, delnode.Value(), 'from low_childs of',
                      node.vertex_id, node.Value())
            if delnode in node.high_childs:
                node.high_childs.remove(delnode)
                print('DeleteLinksToNodeInTable deleting', delnode.vertex_id, delnode.Value(), 'from high_childs of',
                      node.vertex_id, node.Value())
            if delnode in node.low_parents:
                node.low_parents.remove(delnode)
                print('DeleteLinksToNodeInTable deleting', delnode.vertex_id, delnode.Value(), 'from low_parents of',
                      node.vertex_id, node.Value())
            if delnode in node.high_parents:
                node.high_parents.remove(delnode)
                print('DeleteLinksToNodeInTable deleting', delnode.vertex_id, delnode.Value(), 'from high_parents of',
                      node.vertex_id, node.Value())

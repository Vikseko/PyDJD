...
def DJDtoBDD_separated(problem, diagrams, order, logpath, inputs, options):
    ...
    elif sepdjdprepmode == 1:
        robddsatoracle_mode1(prepbinmode, diagrams, problem, bdd_manager, order, djd_prep_time_limit, logpath, cnfname)
    elif sepdjdprepmode == 2:
        robddsatoracle_mode2(prepbinmode, diagrams, problem, bdd_manager, order, djd_prep_time_limit, logpath, cnfname)
    ...
    elif sepdjdprepmode == 7:
        # строим доп БДД из хардтасок от ро-бэкдоров, склеиваем её с другими.
        robddsatoracle_mode7(prepbinmode, diagrams, problem, bdd_manager, order, djd_prep_time_limit, logpath, cnfname,
                             bdd_stop_size, bdd_max_paths, numproc, options.hardtasksfilename)
    elif sepdjdprepmode == 8:
        # строим декартово произведение из хардтасок от ро-бэкдоров, кодируем его в БДД.
        robddsatoracle_mode8(prepbinmode, diagrams, problem, bdd_manager, order, djd_prep_time_limit, logpath, cnfname,
                             bdd_stop_size, bdd_max_paths, numproc, options.hardtasksfilename)
...
def robddsatoracle_mode1(prepbinmode, diagrams, problem, bdd_manager, order, djd_prep_time_limit, logpath, cnfname):
    ...
    if prepbinmode == 0:
        ...
        biggest_bdd_root, max_size = Problem2BDD_dd_format(problem_biggest_djd, bdd_manager, 'DNF')
    ...
    if solveflag is False:
        ...
        result_cnf.to_file(logpath + cnfname + '_djdprep_mode1.cnf')
    else:
        ...
        if biggest_bdd_root.dag_size == 1:
            ...
            if indet_clauses:
                if len(additional_problem) == 0:
                    double_solving = True
                    print(
                        'Additional problen contains paths only from current iteration, so dont need to resolve it. Go to next iteration.')
...
def robddsatoracle_mode7(prepbinmode, diagrams, problem, bdd_manager, order, djd_prep_time_limit, logpath, cnfname, bdd_stop_size, bdd_max_paths, numproc, hardtasksfilename):
    # Версия, в которой пути добавляются в диаграмму пока она не слишком распухнет
    # при этом используется множество хардтасок от ро-бэкдоров (нужен отдельный файл)
...
def robddsatoracle_mode8(prepbinmode, diagrams, problem, bdd_manager, order, djd_prep_time_limit, logpath, cnfname, bdd_stop_size, bdd_max_paths, numproc, hardtasksfilename):
    # Версия, в которой пути добавляются в диаграмму пока она не слишком распухнет
    # при этом используется множество хардтасок от ро-бэкдоров (нужен отдельный файл)
    assert prepbinmode == 1, 'For mode 7 of preprocessing, binarization by apply is needed.'
    djdtobdd_sep_start_time = time.time()
    ...
    ht_bdd_construct_start_time = time.time()
    ht_problems = GetHardTasks(hardtasksfilename)  # получили словарь хардтасок для каждого бэкдора
    ht_cart_prod_st_time = time.time()
    ht_problems_product = GetCartesianProductOfHardTasksDict(ht_problems, 20)
    ht_cart_prod_time = time.time() - ht_cart_prod_st_time
...
def HardTasksDict2BDD_dd_format(ht_problems: dict, bdd_manager, bdd_stop_size, problem_type='DNF', nof_used_backdoors=None):
    ...
    for index, problem in enumerate(keys):
        if 0 < index < nof_used_backdoors:
            ...
            # так как с разных бэкдоров хардтаски должны работать одновременно, то бдд с разных бэкдоров соединяем через AND
            next_problem_root = bdd_manager.apply('and', current_problem_root, new_bd_bdd_root)
            if next_problem_root.dag_size > max_size:
...
def Problem2BDD_dd_format_prepmode4(sortedproblem: list, bdd_manager, bdd_max_size, bdd_max_paths, problem_type='DNF', neg_add_bdd_root=None):
    ...
    # print('Clause:', first_clause)
    # print('Current root expr:', current_problem_root.to_expr())
    final_root = previous_root = current_problem_root
    for index, clause in enumerate(sortedproblem):
        if index > 0:
            # print('Clause:', clause)
            current_clause_root = Clause2BDD_dd_format(clause, bdd_manager, problem_type)
            if problem_type == 'DNF':
                current_problem_root = bdd_manager.apply('or', previous_root, current_clause_root)
            else:
        ...
        final_root = previous_root
    bdd_manager.collect_garbage()
    remain_problem = sortedproblem[stop_index:]
    return final_root, max_size, stop_index, remain_problem
